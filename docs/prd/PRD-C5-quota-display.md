...
| 2026-08-20 | 重构前端摘要区：账号可用/冷却/禁用统计收进紧凑状态框；额度总览改为更大的滚动/每周/每月三条总额度进度条 | 原摘要卡信息密度不均，状态统计占用过多横向空间，额度总览需要更直观表达多账号总分配额度与估算已用比例；数据字段与无阴影无圆角约束保持不变 |
| 2026-08-20 | 再次调整摘要区：将账号状态并入额度总览右侧，同一摘要面板内展示状态统计、额度可用数、三窗口均值与三条额度进度条 | 单独状态面板造成左侧留白，合并后信息层级更集中、横向空间利用率更高 |
