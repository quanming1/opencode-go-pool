...
| 2026-08-20 | 实现完成：断点体系（1024/768/576，收敛旧 900/720）+ 窄屏（≤576）侧栏折叠为顶部 tab + 网格列改 `minmax(0,1fr)` 防子项 min-content 撑破 + keys 表格容器横滚 + 事件时间线可换行；账号卡头部文本省略保护 | E1 开发收口，行为变更留痕（vitest 34 + eslint 0 + build 通过；playwright 五视口 360-1440 无横向溢出、keys 表格可横滚） |
