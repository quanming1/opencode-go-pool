...
| 2026-08-21 | 性能修复：Forwarder 复用单例 httpx.AsyncClient（消除每次请求重新 TLS 握手）；流式成功路径的状态/用量/request 事件改为流发送完毕后补记（首 chunk 前零同步落库，duration 覆盖完整流时长）。实测首 chunk 中位数 2493→2207ms、热连接场景-800ms；pytest 138 + ruff 0（事件时序测试全绿） | perf/B2 流式首字节优化 |
