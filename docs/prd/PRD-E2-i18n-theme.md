...
| 2026-08-20 | 实现完成：自研 i18n（zh/en 字典 + I18nProvider/useI18n + localStorage，未新增依赖）；CSS 变量双主题（:root 浅色 + [data-theme=dark] 深色，全站含 ECharts 动态取色）；header 语言/主题切换控件；13 个组件文案全部 t() 化（顺手修正 header 版本号 v0.1.0→0.3.0） | E2 开发收口，行为变更留痕（vitest 38 + eslint 0 + build 通过；playwright 实测切 EN/Dark 即时生效且刷新持久化） |
